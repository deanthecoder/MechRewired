#[compute]
#version 450
// Vendored for MechRewired from ARez's MIT-licensed compositor lens-effects asset.
...
highp float vector_length_squared(vec2 v) {
...
void lensflares(vec2 uv, vec2 pos, out vec3 sunflare, out vec3 lensflare) {
    ...
    float f2 =
        max(1.0 / (1.0 + 32.0 * vector_length_squared(uvd + 0.8 * pos)), .0) * 00.25;
    float f22 =
        max(1.0 / (1.0 + 32.0 * vector_length_squared(uvd + 0.85 * pos)), .0) * 00.23;
    float f23 =
        max(1.0 / (1.0 + 32.0 * vector_length_squared(uvd + 0.9 * pos)), .0) * 00.21;
